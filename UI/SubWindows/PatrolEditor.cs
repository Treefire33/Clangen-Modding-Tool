...
public class PatrolEditor : Editor
{
	...
	private void DrawPreviewWindow()
	{
		...
		{
			ImGui.Image(PatrolPreviewImg.Handle, new Vector2(600, 500));
			ImGui.SetCursorPos(new Vector2(300, 155));
			ImGui.BeginChild(3, new Vector2(250, 160), ImGuiChildFlags.None);
	...
	private void DrawSelectWindow()
	{
		...
		if(ImGui.Begin("Patrol Select", ImGuiWindowFlags.NoCollapse))
		{
	...
	private string tempRelConstraint = "";
	private int tempRelVal = 0;
	private string tempSkillConstraint = "";
	...
	private void LoadTagEditor()
	{
		...
		{
			if(loadedPatrol is { tags.Count: > 0 })
			{
		...
		if(ImGui.CollapsingHeader("Mix Max Status Editing"))
		{
			if(loadedPatrol != null)
			{
				if(ImGui.BeginChild(2, new(600, 80)))
				{
					...
					{
						if(loadedPatrol.min_max_status is { apprentice: not null })
						{
						...
						if(loadedPatrol.min_max_status is { medicine_cat_apprentice: not null })
						{
						...
						if(loadedPatrol.min_max_status is { medicine_cat: not null })
						{
						...
						if(loadedPatrol.min_max_status is { deputy: not null })
						{
						...
						if(loadedPatrol.min_max_status is { warrior: not null })
						{
						...
						if(loadedPatrol.min_max_status is { leader: not null })
						{
						...
						if(loadedPatrol.min_max_status is { healer_cats: not null })
						{
						...
						if(loadedPatrol.min_max_status is { normal_adult: not null })
						{
						...
						if(loadedPatrol.min_max_status is { all_apprentices: not null })
						{
